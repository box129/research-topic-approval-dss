...
function metricItems(summary, state) {
  ...
  return [
    ...
    { label: 'Similarity Checks', value: formatCount(summary.similarityChecks?.snapshots), helper: summary.similarityChecks?.status === 'available' ? `${formatCount(summary.similarityChecks.highRisk)} higher similarity, ${formatCount(summary.similarityChecks.mediumRisk)} moderate similarity, ${formatCount(summary.similarityChecks.lowRisk)} lower similarity stored snapshots.` : 'Stored similarity snapshot counts are unavailable from the read-only dashboard summary.' }
  ];
}
