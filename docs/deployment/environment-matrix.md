...
| `BULK_HASH_CONCURRENCY` | Backend/admin user import | Optional | No | _unset_ | Explicit bcrypt worker-pool size 1–8. Leave unset for the CPU-derived bounded default; set only after target-capacity measurement because it changes the demonstrated long-running bulk import's CPU/latency profile. | Malformed/out-of-range value fails startup validation; an unsuitable target setting can make the 142-second operation miss its proxy budget. |
