...
| Free demo staging and departmental pilot cost plan missing | FREE MANAGED STAGING PROOF RECORDED FOR FYP/DEMO | Free managed staging evidence, paid pilot procurement review, and updated provider pricing check | Useful | Yes for funded pilot | Project maintainer + department owner | PR #120 adds a free managed staging plan for FYP/demo use and a paid VPS/Docker departmental pilot cost estimate. PR #121 adds provider-specific prep for Vercel, Render, Neon, and Hugging Face Spaces. PR #122 adds a concrete free staging execution checklist and evidence log template. PR #123 records safe Neon project/database setup evidence. PR #124 records safe Hugging Face Space setup evidence with Docker SDK selected. PR #125 prepares a deployable Hugging Face Docker Space package. PR #126 records online SBERT `/health` and `/embed` proof and confirms `SBERT_SERVICE_URL`. PR #127 reviews exact Render backend settings. PR #128 corrects the Render build command, records Render backend online `/api/v1/health` proof, and records successful Neon migration deploy evidence with `migrate status` still pending after `P1001`. PR #129 adds Vercel rewrite config. PR #130 records Vercel frontend root proof, SPA fallback proof, Vercel-to-Render health/readiness proof, and direct Render readiness with `api`, `database`, and `sbert` available. Paid deployment, provider purchase, public production proof, SMTP smoke, credentialed browser smoke, and lecturer validation remain pending. |
...
## Current PR #130 Boundary

PR #130 records docs-only online proof for the free managed staging chain. Vercel frontend root returns HTTP 200, `/login` confirms SPA fallback, Vercel `/api/v1/health` and `/api/v1/readiness` reach Render, direct Render health succeeds, and direct Render readiness reports `ready` with `api`, `database`, and `sbert` available. Render `FRONTEND_URL` was updated to `https://research-topic-approval-dss.vercel.app` and Render was redeployed before final checks. This PR proves the Vercel -> Render -> Neon PostgreSQL + Hugging Face SBERT chain for FYP/demo staging, but it does not prove public production readiness, lecturer validation completion, real SMTP smoke, credentialed browser workflow smoke, monitoring/backup drills, paid departmental deployment, or any application behavior change.
