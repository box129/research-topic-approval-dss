...
async function buildUserImportTemplateWorkbookBuffer() {
  ...
  [
    ...
    'email — required. The account\'s login and recovery email address; it is stored lower-cased and must be unique. Any working address the person can access is acceptable, including a personal one; no particular email domain is required.',
    'role — required. Either "student" or "lecturer". Administrator accounts can never be created by import.',
    'matric_number — optional, students only. The institutional student identifier. Stored upper-cased without spaces; must be unique when provided.',
    '',
