...
describe('Voyage readiness service', () => {
  ...
  test('reports ready from database plus configured Voyage credential without a provider probe', async () => {
    ...
    expect(result.httpStatus).toBe(200); expect(result.body).toMatchObject({ status:'ready', checks:{api:'available',database:'available',semanticProvider:'configured'}, details:{semanticProvider:{provider:'voyage',model:'voyage-4-large',mode:'semantic-only'}} });
  });
